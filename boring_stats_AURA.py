...
def get_all_and_QC_radar_stats(
        ...
        exclusions=None, morning_only=False, radars=[63]):
    ...
    all_radar = [all_obs_radar, QC_obs_radar]
    return all_radar
...
def get_all_regional(
    ...
    all_obs_radar_0 = get_boring_radar_stats(
        save_dir,  exclusions=['simple_duration_cond'], class_thresh=None,
        excl_thresh=None, regime=None, radars=[radars[0]],
        morning_only=morning_only)
    QC_obs_radar_0 = get_boring_radar_stats(
        save_dir, exclusions=exclusions, class_thresh=class_thresh,
        excl_thresh=excl_thresh, regime=None, radars=[radars[0]],
        morning_only=morning_only)

    all_obs_radar_1 = get_boring_radar_stats(
        save_dir,  exclusions=['simple_duration_cond'], class_thresh=None,
        excl_thresh=None, regime=None, radars=[radars[1]],
        morning_only=morning_only)
    QC_obs_radar_1 = get_boring_radar_stats(
        save_dir, exclusions=exclusions, class_thresh=class_thresh,
        excl_thresh=excl_thresh, regime=None, radars=[radars[1]],
        morning_only=morning_only)

    all_obs_radar_2 = get_boring_radar_stats(
        save_dir,  exclusions=['simple_duration_cond'], class_thresh=None,
        excl_thresh=None, regime=None, radars=[radars[2]],
        morning_only=morning_only)
    QC_obs_radar_2 = get_boring_radar_stats(
        save_dir, exclusions=exclusions, class_thresh=class_thresh,
        excl_thresh=excl_thresh, regime=None, radars=[radars[2]],
        morning_only=morning_only)

    all_obs_regional = [
        all_obs_radar_0, QC_obs_radar_0,
        all_obs_radar_1, QC_obs_radar_1,
        all_obs_radar_2, QC_obs_radar_2]

    return all_obs_regional


def plot_regional_seasonal_and_so(
        all_obs_regional, all_radar, fig_dir, suff):
    density = False

    [all_obs_radar, QC_obs_radar] = all_radar
    ...
    compare_offset(
        all_obs_radar, QC_obs_radar, density=density,
        fig=fig, ax=axes[2, :])
...
def plot_all_diurnal(all_radar, fig_dir, suff):
    ...
    compare_time(
        all_obs_radar, QC_obs_radar, density=density,
        title='All Seasons', fig=fig, ax=axes)
...
def plot_all_orientations(all_radar, fig_dir, suff):
...
def plot_all_time_series(time_series_radar, fig_dir, suff):

    fig, axes = plt.subplots(5, 1, figsize=(4, 12))

    cl.init_fonts()

    x = np.arange(0, 48*60+10, 10)

    radar_d_list = [time_series_radar]
    ...
    k = 0
    j = 0
    axes[j].plot(x, radar_d_list[k][0], '', label='Radar Count')

    y_lim = y_lims[j]

    axes[j].set_xlim([0, 240])
    axes[j].set_xticks(np.arange(0, 270, 30))
    # axes[j].set_yticks(
    ...
    axes[j].grid(which='major', alpha=0.5, axis='both')
    axes[j].grid(which='minor', alpha=0.2, axis='y')

    axes[j].ticklabel_format(
    ...
    for j in range(1, axes.shape[0]):
        ...
        [
            means_radar, sig_radar] = [
            np.zeros(len(radar_d[index])) for i in range(2)]
        ...
        axes[j].plot(x, means_radar, '', label='Radar Mean')
        axes[j].fill_between(
    ...
    axes[4].set_xlabel('Time since Detection [min]')
...
def plot_all_eccentricities(all_radar, fig_dir, suff):
    ...
    plt.savefig(
        fig_dir + '/shape_radar_all_{}.png'.format(suff),
        dpi=200, facecolor='w',
        edgecolor='white', bbox_inches='tight')


def plot_all_velocities(all_radar, fig_dir, suff):
    ...
    compare_velocities(
        all_obs_radar, QC_obs_radar, density=density,
        title='All Seasons', labels=False,
        legend=False, fig=fig, ax=axes)
    ...
    plt.savefig(
        fig_dir + '/zonal_radar_active_compare_{}.png'.format(suff),
        dpi=200, facecolor='w', edgecolor='white', bbox_inches='tight')
    ...
    compare_velocities(
        all_obs_radar, QC_obs_radar, density=density,
        title='All Seasons', labels=False,
        legend=False, fig=fig, ax=axes, direction=direction)
    ...
    plt.savefig(
        fig_dir + '/meridional_radar_compare_{}.png'.format(suff),
        dpi=200, facecolor='w', edgecolor='white', bbox_inches='tight')


def get_boring_radar_stats(
        save_dir, exclusions=None, class_thresh=None, excl_thresh=None,
        regime=None, pope_dir=None, radars=[63], morning_only=False,
    ...
    for radar in radars:
        for year in years:
            for month in range(1, 13):
                print('Radar {}, year {}, month {}.'.format(
                    radar, year, month))

                path = save_dir + 'radar_{}/{}/{:02}_{}_{:02}.pkl'.format(
                    radar, year, radar, year, month)
                try:
                ...
                if len(tracks_obj.tracks) > 3:
                    ...
                    try:
                        ...
                        sub_tracks_strat = sub_tracks_all.xs(2, level='level')
...
def plot_counts_regional_seasonal(
        all_obs_radar, QC_obs_radar,
        all_obs_radar_0, all_obs_radar_1, all_obs_radar_2,
        QC_obs_radar_0, QC_obs_radar_1, QC_obs_radar_2,
    ...
    radar = [all_obs_radar[3]]
    QC_radar = [QC_obs_radar[3]]
    ...
    radar = [all_obs_radar_0[3], all_obs_radar_1[3], all_obs_radar_2[3]]
    QC_radar = [QC_obs_radar_0[3], QC_obs_radar_1[3], QC_obs_radar_2[3]]
...
def compare_all_sizes(all_radar, fig_dir, suff):
    ...
    compare_sizes(
        ...
        title='All Seasons')
    plt.savefig(
        fig_dir + '/sizes_radar_all_{}.png'.format(suff),
        dpi=200, facecolor='w',
        edgecolor='white', bbox_inches='tight')


def compare_sizes(
        all_obs_radar, QC_obs_radar, density=True, title=None):
    ...
    ax.flatten()[0].hist(
        [all_obs_radar[0]],
        bins=np.arange(80, 2500, 160), label=['Radar'],
        density=density)
    ...
    ax.flatten()[2].hist(
        [all_obs_radar[1]],
        bins=np.arange(800, 15000, 800), label=['Radar'],
        density=density)
...
def compare_velocities(
    ...
    ax.flatten()[0].hist(
        all_obs_radar[ind],
        bins=np.arange(-20, 22, 2), label=['Radar'],
        density=density)
...
def compare_orientation(
    ...
    ax.flatten()[0].hist(
        all_obs_radar[7], bins=np.arange(0, 391.25, 11.25), label=['Radar'],
    ...
    ax.flatten()[1].hist(
        QC_obs_radar[7], bins=np.arange(0, 391.25, 11.25), label=['Radar'],
        density=density, rwidth=1)
...
def compare_offset(
    ...
    ax.flatten()[0].hist(
        np.array(all_obs_radar[9])/1e3,
        bins=np.arange(0, 52, 2), label=['Radar'],
        density=density, rwidth=1)
...
def compare_time(
    ...
    ax.flatten()[0].hist(
        [a_hour],
        bins=np.arange(0, 25, 1), label=['Radar'],
        density=density, rwidth=1)
...
def get_radar_prop_so_stats(
        save_dir, exclusions=None, class_thresh=None, excl_thresh=None,
        regime=None, pope_dir=None, radars=[42, 63, 77],
        years=range(2012, 2023), morning_only=False):

    if pope_dir is None:
        pope_dir = '/home/student.unimelb.edu.au/shorte1/'
        pope_dir += 'Documents/CPOL_analysis/'

    times = np.arange(0, 48*60+10, 10)
            ...
            for month in range(1, 13):
                ...
                if len(tracks_obj.tracks) > 3:
                    ...
                    for uid in uids:
                        ...
                        sub_tracks_strat = sub_tracks_uid.xs(2, level='level')
                        conv_area = list(sub_tracks_conv['proj_area'].values)
                        strat_area = list(sub_tracks_strat['proj_area'].values)
                        ...
                        cosines_shear = np.cos(
                            np.deg2rad(shear_angle-line_normal))
                        angles_shear = np.arccos(cosines_shear) * 180 / np.pi

                        cosines_prop = np.cos(
                            np.deg2rad(prop_angle-line_normal))
