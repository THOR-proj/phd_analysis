...
def create_categories(tracks_obj):
    ...
    categories = pd.merge(
        eastward_motion, forward_tilt, left_index=True, right_index=True)

    backward_tilt = (
        (-135 >= tracks_obj.system_tracks['sys_rel_tilt_dir'])
        | (tracks_obj.system_tracks['sys_rel_tilt_dir'] >= 135))
    backward_tilt = backward_tilt.rename('backward_tilt')
    categories = categories.merge(
        backward_tilt, left_index=True, right_index=True)

    left_tilt = (
        (45 <= tracks_obj.system_tracks['sys_rel_tilt_dir'])
        & (tracks_obj.system_tracks['sys_rel_tilt_dir'] <= 135))
    ...
    vel_mag = np.sqrt(
        tracks_obj.system_tracks['u_shift']**2
    ...
    not_border = (
        tracks_obj.system_tracks['touch_border'] * 6.25
    ...
    tracks_0 = tracks_obj.tracks[['touch_border', 'proj_area']].xs(
        0, level='level')
    not_border_0 = (
        tracks_0['touch_border'] * 6.25 / tracks_0['proj_area']) < 0.01
